...
class OrderTableCell: UITableViewCell {
    ...
    var order: JKOrder? {
        didSet {
            if let order = order {
                let date = order.pickupDate.formatDate(dateFormat: "dd-MM-yyyy à HH:mm")
    ...
    @IBAction func cancelTapped(_ sender: Any) {
        ...
        alert.addAction(UIAlertAction(title: AULocalized.string("confirm_action"), style: .default, handler: { action in
            if let order = self.order {
                JKMediator.updateOrder(orderId: order.id, userId: order.userId, status: JKOrderStatus.CLIENT_CANCELED.rawValue, success: {
                    if let order = self.order {
